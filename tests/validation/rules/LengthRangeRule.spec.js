...
describe.only('LengthRangeRule', () => {
  ...
  it('throws an error when trying to construct without an array', () => {
    let create = () => new LengthRangeRule("abc");
    expect(create).toThrow("Expected args for LengthRangeRule to be array");
  ...
  it('throws an error when trying to construct with illegal value', () => {
    ...
    expect(create).toThrow('Invalid length range passed to min length rule (5, 4). Max length must be greater than min length');
  });
  ...
  describe("with valid instance", ()=>{
    ...
    it('has a name', () => {
      expect(rule.name).toEqual('length-range');
    });
    ...
    it('returns a resolved promise on successfull validation (non empty value)', async () => {
      return rule.validate("Hell").then(()=>{
        return rule.validate(123);
      });
    });

    it('returns a rejected promise if length is less than min length', async () => {
      return rule.validate("12").catch((e)=> {
