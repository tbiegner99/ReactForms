...
export default () => {
    describe('map object to an array', () => {
        ...
        it('throws an error if object is not an array', () => {
            expect(ArrayUtils.mapToObject.bind(this, 4)).toThrowError(
                'Expected array as first argument'
            );
        });
        ...
        it('returns skips values where key function returns null or undefined', () => {
            const subject = [1, 2, 3, 6, 8, 9];
            const keyFunc = (item) => {
                if (item === 1) return null;
                else if (item % 3 === 0)
                    return String.fromCharCode('a'.charCodeAt(0) + (item / 3 - 1));
                return undefined;
            };
            const valueFunc = (item) => item * 3;
            const result = ArrayUtils.mapToObject(subject, keyFunc, valueFunc);
