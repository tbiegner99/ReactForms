...
export default class ObjectUtilies {
  static filter(objectToMap, doIncludeKey) {
    Assert.toNotBeNullOrUndefined(objectToMap, 'Object to map must be supplied as first argument');
    ...
    const appendKeyToObject = (existingObject, key) => Object.assign(existingObject, { [key]: objectToMap[key] });
  ...
  static map(objectToMap, keyMapFuncion, valueMapFunction) {
    ...
    const appendKeyToObject = (existingObject, key) => Object.assign(existingObject, getValueFromMapperObjectForKey(key));
    return Object.keys(objectToMap)
